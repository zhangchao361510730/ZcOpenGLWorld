...
bool textureImage::InitGlSource() {
    ...
    uint8_t *data = stbi_load(path_picture.c_str(), &width, &height, &nrChannels, 0);
    if (data) {
    ...
    // -------------------------------------------------------------------------------------------
    shaderTool_->use(); // don't forget to activate/use the shader before setting uniforms!
    // either set it manually like so:
    glUniform1i(glGetUniformLocation(shaderTool_->attachId, "texture1"), 0);
