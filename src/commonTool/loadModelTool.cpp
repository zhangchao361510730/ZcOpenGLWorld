...
void loadModelTool::runDrawProcess(ShaderGLSLTool &shader) {
    for(unsigned int i = 0; i < meshes.size(); i++)
        meshes[i].runDrawProcess(shader);
}

// loads a model with supported ASSIMP extensions from file and stores the resulting meshes in the meshes vector.
void loadModelTool::loadModel(string const &path) {
...
void loadModelTool::processNode(aiNode *node, const aiScene *scene) {
...
meshTool loadModelTool::processMesh(aiMesh *mesh, const aiScene *scene) {
    // data to fill
...
vector<Texture> loadModelTool::loadMaterialTextures(aiMaterial *mat, aiTextureType type, string typeName)
{
    vector<Texture> textures;
