...
bool simpleModel::InitGlSource() {
	setCallbackFun_ = simpleModel::setCallBackControl;
...
void simpleModel::framebuffer_size_callback(GLFWwindow* window, int width, int height) {
    glViewport(0, 0, width, height);
}

void simpleModel::mouse_callback(GLFWwindow* window, double xposIn, double yposIn) {
    simpleModel* thiz = (simpleModel*)glfwGetWindowUserPointer(window);
...
void simpleModel::scroll_callback(GLFWwindow* window, double xoffset, double yoffset) {
    simpleModel* thiz = (simpleModel*)glfwGetWindowUserPointer(window);
    thiz->camera_->ProcessMouseScroll(static_cast<float>(yoffset));
}

void simpleModel::setCallBackControl(void*thiz) {
	simpleModel* thiz_ = (simpleModel*)thiz;
    if (thiz_ == nullptr) {
        std::cerr<<__FILE__<<" thiz_ is nullptr"<<std::endl;
    }
    glfwSetFramebufferSizeCallback(thiz_->window, simpleModel::framebuffer_size_callback);
    glfwSetCursorPosCallback(thiz_->window, simpleModel::mouse_callback);
    glfwSetScrollCallback(thiz_->window, simpleModel::scroll_callback);
    glfwSetInputMode(thiz_->window, GLFW_CURSOR, GLFW_CURSOR_DISABLED);
}

void simpleModel::processInput(GLFWwindow *window) {
...
void simpleModel::runDrawProcess() {
    // render loop
...
bool simpleModel::unInitResource() {
